...
func main() {
	...
	connect := dispatcher.ListenMsg{
		ListenerId: &dispatcher.ID{
			PodName:      "pod2",
	...
	for !ready {
		sock, err = client.Listen(context.Background(), &connect)
		if err != nil {
			logger.Warnf("Listen call failed with error: %+v retrying in 120 seconds", err)
